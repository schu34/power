...
class App extends Component {
  ...
  tick () {
    ...
    this.updateHeatMap();
    this.setState({totalHeat: this.state.board.heat.reduce((acc, row)=>{

      return acc + row.reduce((acc, curr)=>{
        return acc + curr;
      }, 0)
  ...
  render () {
    return (
      <div className='App'>
        ...
        <div className='stats'>{'cash: ' + this.state.cash} <br/> {'total heat: '+ this.state.totalHeat }</div>
      </div>
    )
  }
}
